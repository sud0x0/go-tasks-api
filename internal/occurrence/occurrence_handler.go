...
func (h *Handler) handleError(ctx context.Context, w http.ResponseWriter, err error) {
	...
	case errors.Is(err, ErrInvalidAnswerType):
		http.Error(w, "answer type doesn't match task's expected type", http.StatusBadRequest)
	case errors.Is(err, ErrInvalidSelectOption):
		http.Error(w, "invalid select option for this task", http.StatusBadRequest)
	case errors.Is(err, ErrInvalidInput):
		http.Error(w, "invalid input", http.StatusBadRequest)
	case errors.Is(err, ErrAnswerStringTooLong):
		http.Error(w, "answer_string exceeds maximum of 500 characters", http.StatusBadRequest)
	case errors.Is(err, ErrOccurrenceAlreadySuppressed):
		http.Error(w, "occurrence is already suppressed", http.StatusConflict)
